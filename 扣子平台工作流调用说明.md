...
| Event_Update              | 更新收藏夹中的事件                       | 7534551436102254644     |
...
### 4. Event_Update（收藏事件更新）

#### （1）入参要求

- 需传入两个参数：
  - `event_name`：String类型，事件名称（同Event_Collection返回的 `event_name`）
  - `last_timeline`：数组类型，收藏的已经整理好的时间线（同Event_Analysis返回的 `timeline`）
- 示例：

```json
{
    "event_name": "特朗普马斯克决裂事件",
    "last_timeline": [
        {
            "event_phase": "矛盾初现",
            "evidence": "来源于新闻列表",
            "involved_people": "马斯克,特朗普",
            "plot_description": "马斯克狠批特朗普力推的“大而美”税改法案",
            "time": "2025-06-03"
        },
        {
            "event_phase": "公开决裂",
            "evidence": "来源于新闻列表",
            "involved_people": "特朗普,马斯克",
            "plot_description": "特朗普在社交媒体称马斯克“疯了”，因马斯克撤销电动汽车“强制令”，威胁终止其政府补贴和合同，还撤回对马斯克推荐的NASA局长提名；马斯克回怼法案没看过，公开支持弹劾特朗普，称没他帮助特朗普难赢选举，暗示特朗普涉及爱泼斯坦案，宣布即刻退役“龙”飞船，批评特朗普关税政策与税改法案，特斯拉市值蒸发超1500亿美元",
            "time": "2025-06-05"
        }
    ]
}
...
-H "Content-Type: application/json" \
-d '{
  "workflow_id": "7534551436102254644",
  "parameters": {
    "event_name": "特朗普马斯克决裂事件",
    "last_timeline": [
      {
...
- 核心返回字段：`content`（JSON格式字符串），包含事件更新的2个核心变量：
  - `haveProgress`：事件是否需要更新（Boolean类型）
  - `new_timeline`：时间线数组（每个元素含 `time`、`event_phase`等5个字段）
- 示例：（同文档中返回结果，此处省略具体内容）

## 四、通用调用说明

1. 所有工作流均通过 `POST`请求调用，接口地址固定为：`https://api.coze.cn/v1/workflow/run`
2. 请求头必须包含：
